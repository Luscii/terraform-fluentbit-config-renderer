...
- **Single-line object:** Always use commas between key-value pairs.
  ...
  ```
- **Multi-line object:** Each key on a separate line, no commas needed.
  Example:
  ```hcl
  log_sources = [
    {
...
**For all details, advanced usage, and best practices for string templates and the `templatefile()` function, see the [terraform-templatefile skill](/.github/skills/terraform-templatefile/SKILL.md) (required reading for template usage).**

This skill only covers the basic syntax and integration points. For:
- Heredoc and quoted template syntax
- Template directives (`%{ if }`, `%{ for }`, etc.)
- Whitespace trimming and `.tftpl` conventions
- JSON/YAML output from templates
- Directory-based templating
...
- **terraform-functions** skill for using terraform functions
- **terraform-resources** skill for terraform resources
- **terraform-refactoring** skill for safe code restructuring
- **terraform-templatefile** skill for using templates in terraform
- **terraform-values** skill for using and setting inputs (variables, states) and outputs
